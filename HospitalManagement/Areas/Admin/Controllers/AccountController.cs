            ...
            using (var workScope = new UnitOfWork(new HospitalManagementDbContext()))
            {
                ...
                switch (role)
                {
                    ...
                    case RoleKey.Doctor:
                        {
                            _keyElement += " - Bác Sĩ";
                            var accountDoctor = workScope.Accounts
                                .Include(x => x.Doctor).AsQueryable()
                                .Where(x => x.Role == RoleKey.Doctor && !x.IsDeleted).ToList();
                            return View(accountDoctor);
                        }
                    case RoleKey.Patient:
                        {
                            var accountPatient = workScope.Accounts
                                .Include(x => x.Patient).AsQueryable()
                                .Where(x => x.Role == RoleKey.Patient && !x.IsDeleted).ToList();
